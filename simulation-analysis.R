...
setwd('~/your-directory-of-interest/')

# ================================= PART 1 =====================================
# Generating a lookup table for the 51 receptor subtypes
...
colnames(lookup)[which(colnames(lookup) == 'V6')] <- 'receptorNum'
...
# ================================= PART 2 =====================================
# reading, cleaning and plotting example data

examples <- read.table('homology-modelling/examples.txt', sep = ' ')
colnames(examples) <- c('alpha', 'beta', 'delta', 'aa', 'ab', 'ad', 
                        'ba', 'bb', 'bd', 'da', 'db', 'dd', lookup$receptorType)
examples_long <- reshape(data = examples[,13:ncol(examples)],
                         direction = 'long',
                         varying = names(examples)[13:ncol(examples)],
                         v.names = 'numReceptors',
                         timevar = 'receptorType')

# adding receptor subtype labels to the example data
examples_long <- merge(examples_long, lookup, by.x = 'receptorType',
                       by.y = 'receptorNum', all.x =T)

examples_heatmap <- as.matrix(examples[,13:ncol(examples)])
examples_heatmap <- log10(examples_heatmap)
examples_heatmap[which(examples_heatmap == '-Inf')] <- 0

heatmap_plot <- pheatmap(mat = examples_heatmap, 
                         cluster_cols = F, cluster_rows = F, fontsize = 7, 
                         color = colorRampPalette(
                           brewer.pal(n = 7, name = "Blues"))(100),
                         angle_col = '45')


# ================================= PART 3 =====================================
# Extracting information from example data
...
# dbaba= Num 22 (this is the proper ABG 2:2:1) grep(pattern = 'dbaba', x = lookup$combined)
# dbabb= Num. 29 grep(pattern = 'dbabb', x = lookup$combined)
# adbbb = Num 27 (this is the ABBBD type) grep(pattern = 'adbbb', x = lookup$combined)
...
# To identify SOLVABLE but NOT OCCURING receptors, find intersection between ^
...
rowOfConditions_a4b3d <- which( 
  ...
  (sumOurReceptors_a4b3d > sumAllDelta_a4b3d/2) & 
    ...
    # (E) - Both BBBAD and BBBBD are abundant (>1/3 of purified receptors each)**
    (just_data[,27] > sumOurReceptors_a4b3d/3) & 
    (just_data[,45] > sumOurReceptors_a4b3d/3) )


# ================================= PART 5 =====================================
# Plotting the identified conditions for ALPHA4-BETA3-DELTA cell line
...
# To identify SOLVABLE but NOT OCCURING receptors, find intersection between ^
# Remove the solved b3g receptor (BBGBG = 47) and a4b3g (29)
...
rowOfConditions_gamma <- which( 
  ...
  (sumOurReceptors_gamma > sumAllGamma/2) & 
    ...
    # (E) - Both BBGBA and BBGBG are abundant (>1/3 of purified receptors each)**
    (just_data[,29] > sumOurReceptors_gamma/3) &
    (just_data[,47] > sumOurReceptors_gamma/3) )



# ================================= PART 7 =====================================
# Plotting the identified conditions for ALPHA4-BETA3-GAMMA cell line
...
# ================================= PART 9 =====================================
# In this part, we search for identified conditions in which AA, AB, BA and BB 
# interfaces have the same values

# ----- (1) Take the data that satisfies criteria given above
data_gamma <- data[rowOfConditions_gamma, ]
data_gamma$dataset <- 'gamma'
data_delta <- data[rowOfConditions_a4b3d, ]
...
# ================================= PART 9B ====================================
# This part takes 20 random rows from delta and gamma (that meet the conditions)
# and uses only these for plotting
...
# ================================= PART 10 ====================================
# Plotting combined INTERFACE, subunit ABUNDANCE and subtype EXPRESSION data
...
# interfacesGamma_boxplot <- log10(data_gamma[,4:12])
# interfacesDelta_boxplot <- log10(data_delta[,4:12])
...
mean(subset(abundancesDelta_boxplot, interface == 'alpha')$value)
mean(subset(abundancesDelta_boxplot, interface == 'beta')$value)
mean(subset(abundancesDelta_boxplot, interface == 'delta')$value)

mean(subset(abundancesGamma_boxplot, interface == 'alpha')$value)
mean(subset(abundancesGamma_boxplot, interface == 'beta')$value)
mean(subset(abundancesGamma_boxplot, interface == 'delta')$value)
...
# ================================= PART 12a ====================================
# In this part, we search for conditions in which bbabd is the prevalent 
# receptor but the interfaces have the same values as in data_delta
...
# Searching for conditions in which the predominant receptor subtypes, among the
# purifiable receptors, are the DBABA (the 2:2:1)  
# other "solvable" arrangements are kept at minimum.
# additional constraint is that the interfaces are the same as identified in
# data_delta

# The only other condition is that, among purifiable receptors, dbaba is the 
# most prevalent. This is subtype 22
...
# INDEX_notOccurring_babad_ <- intersect(INDEX_gamma, INDEX_ABinterface)
# INDEX_notOccurring_babad_ <- 
#   INDEX_notOccurring_babad_[ ! INDEX_notOccurring_babad_ %in% c('22')]

# ----- (2) Find sums of all receptors with:
# (A) - gamma subunits
# (B) - receptors we observe (BBGBG and BBGBA)
sumAllGamma_babad <- apply(just_data_babad[,INDEX_gamma], 1, sum) # purified 
...
rowOfConditions_babad <- which( 
  ...
  # (A) - Obsesrved receptors are 50% or more of purified receptors
  (data_babad$dbaba > sumAllGamma_babad/2) & 
    
    # (B) - our receptor of interest is >60% of all expressed receptors ***
    (just_data_babad[,c(22)] > 600)  #& 
    
    # # (C) - Observed receptors are above noise level (>33)
    # (just_data_babad[,22] > 33 )  &
    # 
    # # (D) - Not-observed receptors are below the noise level (<33)
    # # (just_data[,INDEX_notOccurring_gamma] < 33) &
    # 
    # (just_data_babad[,5] < 33 ) &
    # (just_data_babad[,10] < 33 ) &
    # (just_data_babad[,11] < 33 ) &
    # (just_data_babad[,14] < 33 ) &
    # (just_data_babad[,15] < 33 ) &
    # (just_data_babad[,16] < 33 ) &
    # (just_data_babad[,21] < 33 ) &
    # (just_data_babad[,23] < 33 ) &
    # (just_data_babad[,25] < 33 ) &
    # (just_data_babad[,26] < 33 ) &
    # (just_data_babad[,29] < 33 ) &
    # (just_data_babad[,30] < 33 ) &
    # (just_data_babad[,34] < 33 ) &
    # (just_data_babad[,36] < 33 ) &
    # (just_data_babad[,37] < 33 ) &
    # (just_data_babad[,40] < 33 ) &
    # (just_data_babad[,41] < 33 )
  )
...
# ================================= PART 13a ====================================
# In this part, we search for conditions in which ABABG is the prevalent 
# receptor but the interfaces have the same values as in data_GAMMA
...
# ================================= PART 14 ====================================
# this part takes a random subset of 100k of simulated conditions and plots a
# heatmap
...
expression_profiles__100k <- pheatmap(
  mat = expression_profiles_sample_10k[,20:ncol(expression_profiles_sample_100k)], 
  cluster_cols = F, cluster_rows = T, fontsize = 7, 
  color = colorRampPalette(brewer.pal(n = 7, name = "Blues"))(100),
  angle_col = '45',
  show_rownames = F,
...
# ================================= PART 16 ====================================
# boxplots for a1bg, dbaba, gba4ba4
...
# interfaces_babag_boxplot$receptor <- 'gamma'
...
# interfaces_babad_boxplot$receptor <- 'delta'

# interfacesCombined_boxplot <- rbind(interfaces_babag_boxplot,
#                                     interfaces_babad_boxplot)
...
# # ================================= PART 17 =====================================
# Entropic mixing effect
data_entropy <- subset(data, data$ab == data$ad &
                         data$ad == data$ba &
                         data$ba == data$bb &
                         data$bb == data$bd &
                         data$bd == data$da &
...
test <- subset(data, data$ab == data$ad &
                         data$ad == data$ba &
                         data$ba == data$bb &
                         data$bb == data$bd &
                         data$bd == data$da &
                 ...
                 data$beta == data$delta)


data_entropy_expression <- pheatmap(
  mat = test[,13:ncol(data_entropy)], 
  cluster_cols = F, cluster_rows = T, fontsize = 7, 
  color = colorRampPalette(brewer.pal(n = 7, name = "Blues"))(100),
  angle_col = '45',
  show_rownames = F,
...
just_data_entropy <- data_entropy[,13:ncol(data_entropy)]
interfaces_entropy <- log10(data_entropy[,4:12])
interfaces_entropy[interfaces_entropy == '-Inf'] <- -4
interfaces_entropy[interfaces_entropy < -1 ] <- -1
abundance_entropy <- data_entropy[,1:3]

###############################################################################
# --------- CHECK WHETHER ENTROPIC MIXING CAN GIVE DISTRIBUTIONS FOR FOR A4B3D
# ----- (1) Find all solvable receptors:
# We can solve any receptor that has (A) a delta-subunit and (B) a b/b interface
INDEX_delta_containing <- grep(pattern = 'd', x = lookup$combined)
INDEX_BBinterface <- grep(pattern = 'bb', x = lookup$combined)
...
# (A) - delta subunits
# (B) - receptors we observe (abbbd and bbbbd)
sumAllDelta_a4b3d <- apply(just_data_entropy[,INDEX_delta_containing], 1, sum) #purified
sumOurReceptors_a4b3d <- apply(just_data_entropy[,c(27, 45)], 1, sum) # solved receptors
...
rowOfConditions_a4b3d_entropy <- which( 
  ...
  (sumOurReceptors_a4b3d > sumAllDelta_a4b3d/2) & 
    ...
    # (C) - Observed receptors are above noise level (>33)
    (just_data_entropy[,27] > 33 ) & 
    (just_data_entropy[,45] > 33 ) &
    
    # (D) - Not-observed receptors are below the noise level (<33)
    # (just_data[,INDEX_notOccurring_a4b3d] < 33 ) &
    (just_data_entropy[,12] < 33 ) &
    (just_data_entropy[,15] < 33 ) &
    (just_data_entropy[,25] < 33 ) &
    (just_data_entropy[,26] < 33 ) &
    (just_data_entropy[,28] < 33 ) &
    (just_data_entropy[,29] < 33 ) &
    (just_data_entropy[,36] < 33 ) &
    (just_data_entropy[,38] < 33 ) &
    (just_data_entropy[,40] < 33 ) &
    (just_data_entropy[,46] < 33 ) &
    (just_data_entropy[,47] < 33 ) &
    (just_data_entropy[,48] < 33 ) &
    
    
    # (E) - Both BBBAD and BBBBD are abundant (>1/3 of purified receptors each)**
    (just_data_entropy[,27] > sumOurReceptors_a4b3d/3) & 
    (just_data_entropy[,45] > sumOurReceptors_a4b3d/3) )
###############################################################################
...
# (A) - gamma subunits
# (B) - receptors we observe (BBGBG and BBGBA)
sumAllGamma_entropy <- apply(just_data_entropy[,INDEX_gamma_containing], 1, sum) # purified 
sumOurReceptors_gamma_entropy <- apply(just_data_entropy[,c(29, 47)], 1, sum) # solved 
...
rowOfConditions_gamma_entropy <- which( 
  
  # (A) - Obsesrved receptors are 50% or more of purified receptors
  (sumOurReceptors_gamma_entropy > sumAllGamma_entropy/2) & 
    
    # (B) - Delta-containing receptors take up >50% of all expressed receptors ***
    (sumAllGamma_entropy > 300)  & 
    ...
    # (E) - Both BBGBA and BBGBG are abundant (>1/3 of purified receptors each)**
    (just_data_entropy[,29] > sumOurReceptors_gamma_entropy/3) &
    (just_data_entropy[,47] > sumOurReceptors_gamma_entropy/3) )
################################################################################
...
# (A) - gamma subunits
# (B) - receptors we observe (BBGBG and BBGBA)
sumAllGamma_entropy <- apply(just_data_entropy[,INDEX_gamma], 1, sum) # purified 
...
rowOfConditions_entropy <- which( 
  
  # (A) - Obsesrved receptors are 50% or more of purified receptors
  (data_entropy$dbaba > sumAllGamma_entropy/2) & 
    
    # (B) - Delta-containing receptors take up >60% of all expressed receptors ***
    (just_data_entropy[,22] > 600)  & 
    
    # # (C) - Observed receptors are above noise level (>33)
    (just_data_entropy[,22] > 33 )  &
    
    # # (D) - Not-observed receptors are below the noise level (<33)
    # # (just_data[,INDEX_notOccurring_gamma] < 33) &
    
    (just_data_entropy[,5] < 33 ) &
    (just_data_entropy[,10] < 33 ) &
    (just_data_entropy[,11] < 33 ) &
    (just_data_entropy[,14] < 33 ) &
    (just_data_entropy[,15] < 33 ) &
    (just_data_entropy[,16] < 33 ) &
    (just_data_entropy[,21] < 33 ) &
    (just_data_entropy[,23] < 33 ) &
    (just_data_entropy[,25] < 33 ) &
    (just_data_entropy[,26] < 33 ) &
    (just_data_entropy[,29] < 33 ) &
    (just_data_entropy[,30] < 33 ) &
    (just_data_entropy[,34] < 33 ) &
    (just_data_entropy[,36] < 33 ) &
    (just_data_entropy[,37] < 33 ) &
    (just_data_entropy[,40] < 33 ) &
    (just_data_entropy[,41] < 33 )
)
